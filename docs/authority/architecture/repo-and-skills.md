...
Post-execution gate invoked by `tdd-execute` for guarded, materially deviated,
weakly verified, public/authority-sensitive, or explicitly requested work.
...
The development flow has proportional execution behavior without adding stage
or risk types: direct work has no case; compact persistent work uses a concise
outcome contract and plan-carried terminal evidence; guarded work adds
current-plan confirmation, exact-baseline deep review, and durable evidence.
Artifact, review, and commit costs are generated by their triggers rather than
imposed by persistence itself.
