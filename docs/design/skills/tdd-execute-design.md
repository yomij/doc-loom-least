...
>
> 目标：定义 `tdd-execute` 如何校验已确认计划、执行 Red-Green-Refactor、处理 TDD 例外、按 Artifact Policy 记录执行证据、更新 handoff 和记录 review_risk 信号。
...
- 如果用户提供了审查 findings 或外部审查结果，本次执行按计划边界处理其中与当前目标相关的返工项。
...
例外记录位置按 Artifact Policy 决定。存在代码 / 行为变化、偏离、失败重试、material review_risk 或恢复需求时写 `execution.md`；纯文档、trivial config、UI 文案等低风险任务如果验证证据可完整写入 `closure.md`，可以不单独生成 `execution.md`。
...
部分执行、等待用户、阻塞、暂停或跨会话继续时应更新短 `handoff.md`。如果 `plan.md` checkbox 和 `execution.md` 已能快速说明下一步，可跳过 `handoff.md`。

### Step 7. Review Risk Signal

根据执行风险记录 `review_risk` 信号，但不建议、不触发、不等待审查对话。

`review_risk` 是给 `docloom-workflow` 消费的风险信号，不是 review 授权，也不是 workflow gate。
...
是否运行 `review` 只能来自用户明确请求，并由 `docloom-workflow` 识别和分派。
...
每项都应有测试、替代验证、代码 diff、用户确认或其他明确证据。无法由 `tdd-execute` 验证的产品、文档或体验条件，标为 `not verified`，交给用户或 `doc-sync-close`；不能默认为通过。
...
`review_risk` 只记录风险等级，不表示审查建议、等待审查或审查结论。详细原因留在 `execution.md` 或 `closure.md`。
...
注意：这只是风险信号，不是 review 授权，不是 workflow gate，也不是下一步建议。

如果用户提供了审查 findings，`tdd-execute` 应在计划边界内处理与当前目标相关的 Critical / Important findings，并在执行记录中映射 finding 与修复证据。返工完成后只记录修复证据和 remaining risk，不写审查状态字段。
