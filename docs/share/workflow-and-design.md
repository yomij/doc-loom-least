...
| 环节 | 驱动它的文档 |
|---|---|
| 当前进度是什么 | `case_state.yaml`（路由信号）+ `plan.md` / `execution.md` / `closure.md` |
...
> **Markdown 是人类可读的主记录，`case_state.yaml` 只是机器可读的路由信号。**

二者冲突时，不静默覆盖 routing signal，而是以 Markdown + git 状态重新推导 phase。这个区分很重要：它让文档始终是"真相"，yaml 只是个加速器——信号错了不影响正确性，因为它能从文档重建。
...
**坑一：以为"更长的 context window = 更好的上下文"。** 把整个仓库塞进 prompt 是本能反应，但相关研究（context rot、Lost in the Middle）表明，关键信息放在长上下文中间时模型利用率显著下降。解法不是堆 context，而是**先路由、再裁剪**：上下文 gate 只读与当前任务相关的最小上下文，`case_state.yaml` 只记路由信号。少而准，胜过多而杂。
...
case 的阶段由 `case_state.yaml`（机器可读路由信号）+ Markdown 产物 + git 状态共同推导。冲突时不静默覆盖，而是以最小可靠信号重新推导：
...
stateDiagram-v2
    ...
    executing --> doc_syncing: 验收完成 + Review pass
    doc_syncing --> closed: closure commit 成功
