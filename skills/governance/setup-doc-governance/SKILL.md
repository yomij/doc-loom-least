...
5. Record entry decisions: repo, docs-route, local, agent-summary, or none.
   For constitution create/migrate/update, record existing agent adapters
   (`AGENTS.md`, `CLAUDE.md`, editor-specific files) as `agent-summary`.
...
Do not create or migrate a constitution only because a conventional file is
absent. Propose a new authority-hosted constitution only when no active
constitution is declared and evidence or user intent requires one. Migrating an
active constitution is a confirmed authority change and must update
index/SSOT/decision log. Keep constitution content to non-negotiable principles;
put workflow rules and procedures in narrower authority docs or skill
implementation. When the constitution changes, point existing agent adapters to
the active constitution and authority entry; do not copy clauses into adapters.
