...
- Agent files such as `AGENTS.md`, `CLAUDE.md`, or editor-specific
  instruction files get only a thin derived pointer.
...
When the active constitution changes, sync adapters to point to it and the
authority index; do not copy clauses or workflow protocol into adapters.
