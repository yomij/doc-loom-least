<!--
Required core: Summary, Acceptance Criteria Status, Tests, Remaining Risks,
Follow-ups, Final Status.

Triggered sections: include each only when it has content.
- Code Changes: Include when code was modified.
- Docs Updated: Include when L2/L3 docs were synced.
- Knowledge Changes: Include when authority_candidate, regression_candidate, or runbook_candidate exists.
- Decisions Reviewed: Include when closure reviewed plan decisions.
- Findings Disposition: Include when user-provided review findings were consumed.
- Authority Changes: Include when authority proposal or patch was recorded.
- Lifecycle Changes Proposed: Include when authority lifecycle status change was proposed.
- Follow-up Updates: Include when existing follow-ups changed status.
- Confirmation Log: Include when closure required explicit user confirmation.
- History: Include when updating an existing closure report.
Do not keep empty headings.
-->
# Closure Report

## Summary

## Acceptance Criteria Status

| Criteria | Status | Evidence |
|---|---|---|

## Tests

| Command | Result | Notes |
|---|---|---|

## Remaining Risks

## Follow-ups

## Final Status

Done / Done with Caveats / Blocked / Cancelled / Superseded / Paused / Abandoned

## Optional Section Menu

Add only the sections needed for this case: Code Changes, Docs Updated,
Knowledge Changes, Decisions Reviewed, Findings Disposition, Authority Changes,
Lifecycle Changes Proposed, Follow-up Updates, Confirmation Log, or History.
Remove this menu after choosing sections.
